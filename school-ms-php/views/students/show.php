...
<div class="row g-3">
  <div class="col-lg-4">
    ...
    <a href="/students" class="btn btn-sm btn-link mt-2">
      ...
      <div class="card-header py-3 d-flex justify-content-between align-items-center">
        ...
        <div class="d-flex gap-2">
          ...
          <button class="btn btn-sm btn-primary"
                  data-bs-toggle="modal" data-bs-target="#linkParentModal">
      ...
      <div class="list-group list-group-flush">
        <?php if (empty($parents)): ?>
          <div class="list-group-item text-muted small py-3">
            No parents linked yet. Use the button above to link one.
          </div>
        <?php else: foreach ($parents as $p): ?>
          <div class="list-group-item d-flex justify-content-between align-items-center">
            <div>
              <div class="fw-semibold small"><?= htmlspecialchars($p['Name'] ?? $p['name'] ?? '') ?></div>
              <div class="text-muted" style="font-size:.75rem">
                <?= htmlspecialchars($p['Email'] ?? $p['email'] ?? '') ?>
                <?php if (!empty($p['Phone'] ?? $p['phone'] ?? '')): ?>
                  · <?= htmlspecialchars($p['Phone'] ?? $p['phone']) ?>
                <?php endif; ?>
              </div>
            </div>
            <a href="/parents/<?= $p['ID'] ?? $p['id'] ?>" class="btn btn-sm btn-outline-primary">
              <i class="bi bi-eye"></i>
            </a>
          </div>
      ...
      <div class="modal-body">
        <form method="POST" action="/parents/link-student">
          ...
          <div class="mb-3">
            ...
            <select name="parent_id" class="form-select" required>
              ...
              <?php foreach ($allParents as $p): ?>
                <option value="<?= $p['ID'] ?? $p['id'] ?>">
                  <?= htmlspecialchars($p['Name'] ?? $p['name'] ?? '') ?>
                  <?php if (!empty($p['Phone'] ?? $p['phone'] ?? '')): ?>
                    — <?= htmlspecialchars($p['Phone'] ?? $p['phone']) ?>
                  <?php endif; ?>
                </option>
            ...
            <div class="form-text">
              Don't see the parent? <a href="/parents/create" target="_blank">Create one first</a>.
            </div>
          </div>
          <div class="mb-3">

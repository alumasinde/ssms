...
// FIX: all methods now use context.WithTimeout(5s) to prevent goroutine
// leaks when the DB is slow or unresponsive under load.
const financeTimeout = 5 * time.Second
...
func (r *FinanceRepository) CreateFeeType(ft *models.FeeType) error {
	ctx, cancel := context.WithTimeout(context.Background(), financeTimeout)
	defer cancel()
...
func (r *FinanceRepository) ListFeeTypes(schoolID int64) ([]models.FeeType, error) {
	...
	return list, r.db.SelectContext(ctx, &list,
		`SELECT * FROM fee_types WHERE school_id=?`, schoolID)
}
...
func (r *FinanceRepository) GetInvoiceByID(id int64) (*models.FeeInvoice, error) {
	...
	return &inv, r.db.GetContext(ctx, &inv,
		`SELECT * FROM fee_invoices WHERE id=?`, id)
}
...
func (r *FinanceRepository) UpdateInvoiceStatus(id int64, status string) error {
	...
	_, err := r.db.ExecContext(ctx,
		`UPDATE fee_invoices SET status=? WHERE id=?`, status, id)
	return err
}
...
func (r *FinanceRepository) TotalPaidForInvoice(invoiceID int64) float64 {
	...
	r.db.GetContext(ctx, &total, //nolint:errcheck
		`SELECT COALESCE(SUM(amount_paid),0) FROM fee_payments WHERE invoice_id=?`, invoiceID)
	return total
}
...
func (r *FinanceRepository) ListStudentsByClass(classID int64) ([]int64, error) {
	...
	defer cancel()
	var ids []int64
	return ids, r.db.SelectContext(ctx, &ids,
		`SELECT id FROM students WHERE class_id=? AND is_active=1`, classID)
}
...
func (r *FinanceRepository) CreateDiscount(d *models.FeeDiscount) error {
	...
	res, err := r.db.ExecContext(ctx,
		`INSERT INTO fee_discounts (school_id,student_id,fee_type_id,term_id,label,discount_pct,discount_amt,approved_by,is_active)
		 VALUES (?,?,?,?,?,?,?,?,1)`,
		d.SchoolID, d.StudentID, d.FeeTypeID, d.TermID,
		d.Label, d.DiscountPct, d.DiscountAmt, d.ApprovedBy)
	if err != nil {
		return err
	}
	id, _ := res.LastInsertId()

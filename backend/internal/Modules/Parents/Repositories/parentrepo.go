...
func (r *ParentRepository) FindByID(id int64) (*models.ParentDetail, error) {
	...
	return &p, r.db.Get(&p, `SELECT p.*,u.first_name, u.last_name, u.email FROM parents p JOIN users u ON u.id=p.user_id WHERE p.id=?`, id)
}

func (r *ParentRepository) ListBySchool(schoolID int64) ([]models.ParentDetail, error) {
	var list []models.ParentDetail
	return list, r.db.Select(&list, `SELECT p.*,u.first_name,u.last_name,u.email FROM parents p JOIN users u ON u.id=p.user_id WHERE p.school_id=? ORDER BY u.first_name, u.last_name`, schoolID)
}
...
func (r *ParentRepository) GetParentsByStudent(studentID int64) ([]models.ParentDetail, error) {
	...
	return list, r.db.Select(&list, `
		SELECT p.*,u.first_name,u.last_name,u.email FROM parents p
		JOIN users u ON u.id=p.user_id
		JOIN parent_student ps ON ps.parent_id=p.id
		WHERE ps.student_id=? ORDER BY u.first_name, u.last_name`, studentID)
}

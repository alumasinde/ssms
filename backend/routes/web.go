...
func Setup(db *sqlx.DB, allowedOrigins []string) http.Handler {
	...
	// FIX: 60*1e9 is float64 — must use time.Duration constants
	authLimiter := ratelimit.New(10, time.Minute)        // 10 req/min per IP on auth
	apiLimiter  := ratelimit.New(300, time.Minute)       // 300 req/min per IP on API
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		// FIX: Auth RegisterRoutes now takes limiter as 3rd arg (from our fix)
		// All other modules still use the original (r, db) signature
		auth.RegisterRoutes(r, db, authLimiter)
